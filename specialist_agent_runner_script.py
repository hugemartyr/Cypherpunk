def generate_persistent_runner_content(model_id: str, port: int, task_type: str, log_file: str) -> str:
    """
...
""".replace("MODEL_ID = \"gpt2\"", f"MODEL_ID = \"{model_id}\"") \
                                   .replace("TASK_TYPE = \"auto\"", f"TASK_TYPE = \"{task_type}\"") \
                                   .replace("SEED_PHRASE = f\"{MODEL_ID}_seed_phrase_123456\"", f"SEED_PHRASE = f\"{model_id}_seed_phrase_123456\"") \
                                   .replace("# Log file path placeholder", f'LOG_FILE = r"""{log_file}"""') \
                                   .replace("PORT = 8001", f"PORT = {port}")