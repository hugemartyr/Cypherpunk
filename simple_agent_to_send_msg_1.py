...
async def startup_handler(ctx: Context):
    ...
    print(f"Agent1 {ctx.agent.address} started and ready to send messages.")
    ...
    initial_message = HFManagerChat(
        ChatMessage=ChatMessage(
            ...
            msg_id=uuid4(),
            content=[TextContent(type="text", text="generate text-summarization history of artificial intelligence in brief ")],
        ),
        caller_Agent_address=ctx.agent.address
    )
