...
def generate_runner_script_content() -> str:
    """
    Agent A uses this tool to generate the code for the task-specific model runner.
    This script loads a real Hugging Face model/pipeline and executes the given prompt.
    """
...
def run_agent_A(model_id: str, prompt: str, task_type: str = "auto") -> Dict[str, Any]:
    ...
    1. Generates the runner script
    2. Writes it to a file
    3. Executes the file using subprocess
    ...
    try:
        ...
        script_content = generate_runner_script_content()
        
        # 2. Write the script to a file
        with open(filename, "w", encoding="utf-8") as f:
            f.write(script_content)
        print(f"\n[AGENT A] Created runner script: {filename}")
        
        # 3. Execute the script using subprocess
        print(f"[AGENT A] Executing model: {model_id}")
        print(f"[AGENT A] This may take 30-120 seconds for model download and loading...")
        ...
        print(f"[AGENT A] Execution completed (exit code: {result.returncode})")
        
        # Debug: Show stderr (contains loading messages)
        if stderr:
            print(f"\n[AGENT A] Model loading logs:")
        ...
        try:
            ...
            if response_data.get("success"):
                return {
                    ...
                    "model_id": model_id,
                    "prompt": prompt,
                    "model_response": response_data.get("output", ""),
                    "raw_output": stdout,
                    "stderr": stderr
                }
            else:
            ...
            return {
                "success": False,
                "error": f"Failed to parse model output: {str(e)}",
                "raw_output": stdout,
                "stderr": stderr
            }
...
def main():
    print("=" * 70)
    print("    Agent A: HuggingFace Model Orchestrator")
    print("=" * 70)
    print("This agent dynamically creates and executes a runner script")
    print("to load and run REAL HuggingFace models.\n")
    
    print("Example models (lightweight for testing):")
    print("  Text Generation:")
    print("    - gpt2")
    print("    - distilgpt2 (faster, smaller)")
    print("  Summarization:")
    print("    - facebook/bart-large-cnn")
    print("    - sshleifer/distilbart-cnn-12-6 (faster)")
    print("  Sentiment Analysis:")
    print("    - distilbert-base-uncased-finetuned-sst-2-english")
    print("  Translation:")
    print("    - Helsinki-NLP/opus-mt-en-de (English to German)")
    print()
    ...
    prompt = input("Enter your prompt/text: ").strip()
    if not prompt:
        print("Error: Prompt cannot be empty")
        sys.exit(1)
    
    print("\nTask type (press Enter for auto-detection):")
    print("  Options: text-generation, summarization, translation, sentiment-analysis")
    task_type = input("Task type: ").strip() or "auto"

    print("\n" + "-" * 70)
    
    # Execute Agent A
    result = run_agent_A(model_id, prompt, task_type)
    
    # Display results
    print("=" * 70)
    print("AGENT A FINAL RESPONSE")
    print("=" * 70)
